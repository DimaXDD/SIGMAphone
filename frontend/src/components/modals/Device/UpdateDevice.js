...
const UpdateDevice = observer((props) => {
  ...
  const updateDevice = () => {
    ...
    if (validation.status) {
      ...
      updateOneDevice(formData, device.updateId).then((data) => {
        props.onHide();
    ...
    <Modal show={props.show} onHide={props.onHide} centered>
      ...
      <Modal.Body>
        <Form>
          ...
          {Array.isArray(device.updateInfo)
            ? device.updateInfo.map((i) => (
      ...
      <Modal.Footer className="justify-content-center">
        <Button color="error" onClick={props.onHide}>
          Закрыть
        </Button>
        <Button color="success" onClick={updateDevice}>
          Изменить
        </Button>
      </Modal.Footer>
    </Modal>
