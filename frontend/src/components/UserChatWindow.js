...
const UserChatWindow = observer((props) => {
  ...
  function connect() {
    socket.current.socket = new WebSocket("wss://localhost:5000/websockets");
    let message;
    ...
    socket.current.socket.onmessage = (event) => {
      ...
      if (message.event === "connection") {
        ...
        const result = window.confirm(
          "Администратор ответил на интересующие вас вопросы?"
        );
        if (result) {
          messageOut = {
            id: socket.current.chatId,
  ...
  const sendMessage = async () => {
    let message = {
      ...
      adminname:
        socket.current.adminname !== "" ? socket.current.adminname : null,
      event: "message",
      state: socket.current.userState,
      from: "client",
      to: "server",
    ...
    if (socket.current.userState === "answering") message.to = "admin";
  ...
  return (
    <Card className="p-4 w-100 h-100  ">
      <Card.Title>
        {" "}
        Чат технической поддержки. Администратор:{" "}
        {adminname ? adminname : `(ожидание администратора...)`}
      </Card.Title>
            ...
            <Box
              style={
                mess.from === "client"
                  ? {
                      borderRadius: "50px",
                      backgroundColor: "rgb(170, 255, 128)",
                    }
                  : {
                      borderRadius: "50px",
              ...
              className={
                mess.from === "client"
                  ? "align-self-end neo mb-2 p-2 px-5"
                  : "mb-2 p-2 px-5  neo  align-self-start"
              }
            >
