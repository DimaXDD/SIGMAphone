...
const BasketTable = observer(({ tableCallback }) => {
  ...
  return (
    <Container className="p-0 p-sm-0    p-md-0">
              ...
              <tr key={basketDeviceItem.id}>
                <td className="col-1">
                  <Button
                    ...
                    className="p-0 m-0"
                    onClick={() =>
                      handleDeleteBasketDevice(basketDeviceItem.id).then(() =>
                ...
                <td className="col-2 ">
                  <Image
                    ...
                    src={
                      ...
                      "static/" +
                      basketDeviceItem.device.image
                    }
                  />
                </td>
                <td className="col-9">
                  <h4 className={"m-1"}>{basketDeviceItem.device.price}$</h4>
                  <h5 className={"fw-light m-1 p-0"}>
                    {basketDeviceItem.device.brand.name} -{" "}
                    {basketDeviceItem.device.name}
                  </h5>
                  <h6 className={"fw-light m-1 text-muted"}>
                    {basketDeviceItem.device.type.name}
                  </h6>
                </td>
              </tr>
            ))
